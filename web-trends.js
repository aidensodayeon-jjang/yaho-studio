...
const PROMPT = `웹 검색으로 "외국인 방한 한국관광 핫트렌드 TOP 20"을 조사해 정리하세요.
샤오홍슈/틱톡/레딧/인스타/블로그/뉴스/인바운드 플랫폼(크리에이트립·클룩) 리포트를 폭넓게 참고하세요.

[원칙]
- 외국인이 실제로 열광하는 핫트렌드를 **최대한 폭넓게 20개** 뽑아 화제도(heat, 0~100) 순위를 매기세요.
- 두 종류를 모두 충분히 포함:
  · "popular" = 이미 정착된 인기 (예: 명동 K-뷰티 쇼핑, 한류 성지순례, 한식 미식, 한강 피크닉, 전통시장 먹방, 한복 체험) — 8~10개.
  · "rising" = 최근 급상승 신조어·미세 트렌드 (예: 부산병, 케어케이션, 1인 세신, 퍼스널컬러, 성수동 카페, 두피 스파, 뮷즈 오픈런) — 10~12개.
- 정적 대형 관념어(경복궁·남산타워 단독)보다 구체적이고 상품화 가능한 형태로.
- 의미가 겹치는 키워드는 하나로 합치고, 서로 다른 20개 안팎을 확보하세요.
- 각 항목은 인바운드 관광상품으로 만들 수 있어야 함.
